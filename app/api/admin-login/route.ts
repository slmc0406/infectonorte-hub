...
import { absoluteUrl, runtimeEnv } from "@/lib/runtime";
...
export async function POST(request: Request) {
  ...
  if (!expected || !secret) return NextResponse.redirect(absoluteUrl("/admin/login?error=1", request), 303);
  ...
  if (limit.blocked) return NextResponse.redirect(absoluteUrl("/admin/login?error=1", request), 303);
  if (!isAdminEmail(email) || !(await safeSecretMatch(password, expected))) {
    ...
    throwIfError(error);
    return NextResponse.redirect(absoluteUrl("/admin/login?error=1", request), 303);
  }
  ...
  return NextResponse.redirect(absoluteUrl("/admin", request), 303);
}
